...
export default function ReelStoryboardToolPage() {
  ...
  const runGenerate = useCallback(async (bId: string, sel: StrategySelection) => {
    ...
    setUpdating(true);
    setError(null);
    const result = await generateContent(bId, { ...sel, directionLevel: sel.directionLevel || "director" });
    setUpdating(false);
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      runGenerate(brandId, { ...selection, directionLevel: selection.directionLevel || "director" });
    }, DEBOUNCE_MS);
    return () => {
      if (debounceRef.current) clearTimeout(debounceRef.current);
    };
      ...
      <div className="flex flex-1 gap-6 overflow-hidden">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="mb-3 flex flex-wrap items-center gap-3">
            ...
            <button
              type="button"
              onClick={() => runGenerate(brandId, { ...selection, directionLevel: selection.directionLevel || "director" })}
