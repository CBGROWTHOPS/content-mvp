...
export default function ReelStoryboardToolPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div>
        <h1 className="text-xl font-semibold text-zinc-100">Reel Storyboard</h1>
        <p className="mt-1 text-sm text-zinc-400">
          Shot-by-shot breakdown with timing, movement, and scene notes.
        </p>
        <div className="mt-3 rounded border border-zinc-800/50 bg-zinc-900/30 px-3 py-2 text-xs text-zinc-500">
          <strong className="text-zinc-400">How it works:</strong> Pick brand and preset. Format, Duration, Tone, Style appear in the output. Generate, then Load last or Save to Drive.
        </div>
      </div>
      ...
      <div>
        <ReelBlueprintPanel data={output?.reelBlueprint ?? null} strategySelection={selection} />
      </div>
    </div>
  );
}
